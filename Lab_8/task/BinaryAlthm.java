...
public class BinaryAlthm {
    Node root;
   	...
   	public int hight(int key){	//высота узла
    	Node current = find(key);
    	return hightAlgoritm(current); 
	  }
   	public int layer(int key){	//уровень узла
   	...
   	public static int maxWay(Node current, String str){
   		int tempFirst = 0;
   		int tempSecond = 0;
   		if(str.equals("родитель")){
   			if(current.left != null) tempFirst = maxWay(current.left, "родитель");
   			if(current.right != null) tempSecond = maxWay(current.right, "родитель");
   		}
   		if(str.equals("лев.")){
   			if(current.parent != null) tempFirst = maxWay(current.parent, current.data > current.parent.data ? "прав." : "лев.");
   			if(current.right != null) tempSecond = maxWay(current.right, "родитель");
   		}
   		if(str.equals("прав.")){
   			if(current.left != null) tempFirst = maxWay(current.left, "родитель");
   			if(current.parent != null) tempSecond = maxWay(current.parent, current.data > current.parent.data ? "прав." : "лев.");
   		}
   	...
   	public void task(){
   		...
   		for(int i = 0; i < lists.size(); i++){
   			if(maxWay(lists.get(i), lists.get(i).data > lists.get(i).parent.data ? "прав." : "лев.") >
   			maxWay(maxNode, maxNode.data > maxNode.parent.data ? "прав." : "лев.")) {
   				maxNode = lists.get(i);
   			}
   			else if(maxWay(lists.get(i), lists.get(i).data > lists.get(i).parent.data ? "right" : "лев.") ==
   			maxWay(maxNode, maxNode.data > maxNode.parent.data ? "прав." : "лев.")) {
